...
export function ChoiceCard({
  ...
  return (
    <button
      ...
      onClick={onSelect}
      className={cn(
        "min-h-full w-full cursor-pointer rounded-[var(--animal-radius)] border-2 p-5 text-left transition-all duration-200 disabled:cursor-default disabled:opacity-70",
        selected
    ...
    >
      ...
      <p className="echo-long-text mt-4 font-mono">{content}</p>
      {detail && <p className="mt-4 max-w-[76ch] text-sm leading-7 opacity-75">{detail}</p>}
      {children}
    </button>
  );
}
