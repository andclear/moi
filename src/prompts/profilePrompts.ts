  ...
  return [
    {
      ...
      content: [
        ...
        "角色最终可能导出为中文 YAML/JSON，所以字段内容必须保持中文表达；叙事字段中用户必须使用 {{user}}，角色必须使用真实姓名。",
    ...
    },
    {
      ...
      content: [
        ...
        "## 世界观 与 ## 开场白 必须暂时写“尚未明确”，不要提前生成世界设定或开场白。如果已知用户的世界观倾向，可以简要记录下来",
        "必须为 TA 生成一个具体姓名，并把姓名、用户最初输入、TA 的性别、TA 的年龄写进档案，优先放在 ## 最初的印象；如果年龄未填写，必须明确写“年龄：未填写”。",
        "姓名必须写成“姓名：具体姓名”。姓名、性别和年龄是基础事实，不能省略、改写成不确定，也不能在后续内容中丢失；后续只有用户手动编辑时才能更改。",
...
const stageInstructions: Record<ProfileStageId, string[]> = {
  ...
  diary: [
    ...
    "日记里必须有 3 处关键内容被遮挡，分别用 [[blank_1]]、[[blank_2]]、[[blank_3]] 放在正文中。",
        ...
        "blanks[i].key 必须分别是 blank_1、blank_2、blank_3，不能为 null，不能缺失，且必须和 diaryText 里的占位符完全一致。",
        "blanks[i].options 里的 label 和 meaning 都必须是非空字符串；如果某个选项写不出来，就重新生成该选项，不要用 null 或空字符串占位。",
        "返回格式：",
  ...
  return [
    {
      ...
      content: [
        ...
        "JSON 字段必须使用正确类型；字符串字段必须是非空字符串，数组字段必须是数组。禁止输出 null；可选字段没有内容时直接省略。",
        "写作原则：简单、具体、好懂。不要为了文笔牺牲清晰度。",
        "每个候选都必须尊重当前资料和已确认选择。",
        "如果文本涉及用户，请使用 {{user}}；如果文本涉及角色，请使用角色真实姓名。",
      ].join("\n"),
    },
    {
      role: "user",
  ...
  return [
    ...
    },
    {
      ...
      content: [
        ...
        "必须保留并延续当前岛民档案中的姓名、用户最初输入、性别、年龄。姓名、性别和年龄是基础事实，不得丢弃、改名、改性别或改年龄；年龄未填写时继续写“年龄：未填写”。",
        "每段 1-3 句即可，避免长篇解释。",
        "",
