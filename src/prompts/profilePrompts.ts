  ...
  return [
    {
      ...
      content: [
        ...
        "角色最终可能导出为中文 YAML/JSON，所以字段内容必须保持中文表达；叙事字段使用 {{user}} 和 {{char}}，但姓名字段必须写具体中文姓名，不得写 {{char}}。",
    ...
    },
    {
      ...
      content: [
        ...
        "姓名必须写成“姓名：具体姓名”，不得使用 {{char}}、TA、未明确或占位符。姓名、性别和年龄是基础事实，不能省略、改写成不确定，也不能在后续内容中丢失；后续只有用户手动编辑时才能更改。",
