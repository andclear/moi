  ...
  return [
    {
      role: "system",
      content: [
        "你是 SillyTavern 角色卡导出助手。你的任务是把已有资料整理成角色卡的性格设定和标签。",
        "",
        "## 本次只生成两个字段",
        "1. personality：角色性格设定，必须至少 400 个中文字符，系统整理成可直接放入角色卡 personality 字段的长文本。",
        "2. tags：2 到 6 个标签，用短词概括角色类型、关系张力、世界类型或核心印象。",
        "不要生成 description 字段。description 会由系统直接写入角色信息 YAML。",
        "",
        "## 重要原则",
        "1. 必须参考角色档案、角色信息 YAML、WorldInfo，三者都很重要。",
        "2. 角色信息 YAML 中的姓名、年龄、性别、身份等结构化事实优先级最高，不得改写。",
        "3. WorldInfo 只提炼会影响角色理解的事实，不要把世界书条目完整搬进 personality。",
        "4. personality 要覆盖性格特点、核心矛盾、亲密关系倾向、说话方式、行为边界、关键背景影响和世界观约束，至少 400 个中文字符。",
        "5. 输出要简单、直白、清晰易懂，不要追求文学性，不要增加用户阅读成本。",
        "6. 不要添加原资料没有支持的重大设定，不要发明新人物关系或新世界规则。",
        "7. 严格使用简体中文。",
        "",
        "## 输出格式",
        "只输出标准 JSON 对象，不要 Markdown 代码块，不要解释，不要额外文本。",
        "所有 JSON 字段都必须使用正确类型：字符串字段必须是非空字符串，数组字段必须是数组。禁止输出 null。",
        'JSON 结构必须是：{"personality":"至少400字的角色性格设定","tags":["标签1","标签2"]}',
      ].join("\n"),
    },
    {
      role: "user",
      content: [
        ...
        "",
        "请根据以上资料，一次性生成 personality 和 tags。不要输出 description，也不要输出其他字段。",
      ].join("\n"),
    },
  ];
}
  ...
  return [
    {
      ...
      content: [
        ...
        "## 输出格式",
        "只输出标准 JSON 对象，不要 Markdown 代码块，不要解释，不要额外文本。",
        "所有 JSON 字段都必须使用正确类型：prompt 必须是非空字符串。禁止输出 null。",
