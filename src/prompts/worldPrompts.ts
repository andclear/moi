  ...
  return [
    {
      ...
      content: [
        ...
        "",
        "## 合并与拆分",
        "1. entry_count 是必须输出的条目数量。若用户要求的是强相关的连续结构、阶段变化、等级表、时间线、仪式流程、好感度变化、权限层级或同一系统的多个截断点，优先合并为完整条目；如果合并后不足 entry_count，则围绕该主题补足必要的配套条目，例如制度来源、执行地点、相关人物、例外规则或代价链条。",
        "2. 如果多个主题彼此独立、触发 keys 不同、插入位置或常驻规则不同，应拆成多个世界书条目，直到输出数量严格等于 entry_count。",
        "",
        "## 格式与输出",
