...
const modeDescriptions: Record<TrialMode, string> = {
  ...
  diary: "日记对话：引用核心矛盾或私密记忆，检验过去誓言与当前行为之间的张力。",
  silent: "沉默证词：问题要让正式回答与内心独白形成合理反差。",
};
  ...
  return [
    {
      role: "system",
      content: [
        "你是角色一致性终审员，任务是生成一份温和但有效的角色一致性测试问卷。",
        "问题必须服务于辨认 {{char}} 是否稳定，不要制造压迫感，不要像刑讯。",
    ...
    },
    {
      ...
      content: [
        ...
        `已选开场白：\n${input.selectedGreeting?.content ?? "尚未选定。请基于角色档案测试。"}`,
      ].join("\n\n"),
    },
  ];
}
  ...
  return [
    {
      role: "system",
      content: [
        "你将扮演角色 {{char}} 完成终审问卷，同时给出可供创作者判断的内心独白与 OOC 风险。",
