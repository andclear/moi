  ...
  return [
    {
      ...
      content: [
        ...
        "3. 状态栏类内容默认使用 <details><summary>...</summary><statusblock>...</statusblock></details>；正则只匹配 <statusblock>...</statusblock> 内的内容，不吞掉外层 details。",
        "4. 如果是论坛贴、短信、公告、票据、聊天气泡等随剧情触发的美化，可以使用更合适的自定义标签，但标签名必须稳定、易匹配。",
        "5. formatted_original_text 中的字段名、换行、{{user}}、{{char}} 必须和 regex 捕获顺序严格一致。",
        "",
        "## 前端代码要求",
        "1. html 必须是完整可执行片段，包含唯一父级 class、<style>，必要时包含 <script>。",
        "2. 支持 HTML、CSS、JavaScript；应主动加入有意义的点击或触控交互，例如标签切换、展开内容、数值高亮、局部动画、状态切换。",
        "3. 手机端优先，使用百分比、max-width、flex/grid 和媒体查询；禁止使用 vh 单位。",
        "4. 不能依赖 hover 才能看见关键信息；重要交互必须绑定 click 或 touch。",
        "5. 动画用 @keyframes 或 JavaScript 切换 class；不要使用 CSS transition。",
        "6. 主容器 pointer-events: none；可点击子元素 pointer-events: auto。",
