  ...
  return [
    {
      ...
      content: [
        ...
        "严格使用简体中文。最终开场白必须保留字面占位符 {{user}} 代表用户，但角色必须使用角色信息 YAML 中的真实姓名，不得使用 {{char}}。",
        "严格代称规则：凡是涉及用户的主语、宾语、称呼、动作承受者或身体部位所属者，都必须直接写 {{user}}，不得写“你”“对方”“那个人”“客人”“玩家”等代称。",
        `严格代称规则：凡是涉及角色本身的主语、宾语、称呼、动作承受者或身体部位所属者，都必须直接写 ${characterName}，不得写 {{char}}、“我”“他”“她”“TA”“少女”“男人”“女人”等代称。`,
    ...
    },
    {
      ...
      content: [
        "## 任务",
        `为角色 ${characterName} 撰写引人入胜的开场信息，以开启与 {{user}} 的角色扮演场景。`,
        "",
        "## 上下文数据",
        ...
        "",
        "## 关键约束与质量标准",
        `1. 变量协议：最终输出中必须使用确切字符串 {{user}} 代表用户；最终输出中禁止出现 {{char}}，所有角色位置必须写 ${characterName}。`,
        `2. 第一人称：可以聚焦 ${characterName} 的主观感受和内心反应，但所有涉及角色本身的位置仍必须写 ${characterName}，不得写“我”。`,
        "3. 第二人称：可以聚焦 {{user}} 的所见、所闻、所感，但所有涉及用户的位置仍必须写 {{user}}，不得写“你”。",
        `4. 第三人称：采用电影化客观镜头，但所有涉及角色本身的位置仍必须写 ${characterName}，不得写他/她/TA。`,
        "5. 篇幅必须接近目标字数，不要明显过短。",
        ...
        `11. ${characterName} 是独立个体，有自己的事业、社交圈、欲望和边界，不应只围着 {{user}} 转。`,
        `12. ${characterName} 与 {{user}} 的互动建立在人格平等上，张力来自观点碰撞、未完成的事和真实摩擦。`,
        "",
        "## 输出流程",
        `生成开场白前，用 <cot></cot> 简短自检：分析请求、确认视角、设计钩子、检查禁用套路、确认所有涉及用户的位置都写 {{user}}、所有涉及角色本身的位置都写 ${characterName}、检查所有对话是否使用双引号。`,
        "",
        "## 输出格式",
        "只输出纯文本开场白，不要输出 JSON。",
