  ...
  return [
    {
      ...
      content: [
        ...
        "只输出一个 JSON 对象，不要输出解释、自检、思考过程或任何 JSON 以外的文字。",
        "所有 JSON 字段都必须使用正确类型：字符串字段必须是非空字符串，布尔字段必须是 true/false，数组字段必须是数组。禁止输出 null；可选字段没有内容时直接省略。",
    ...
    },
    {
      ...
      content: [
        ...
        "输出格式必须是 JSON 对象，不要输出 JSON 以外的任何内容。",
        "JSON 格式：",
