...
| 登岛问卷 System | `你可以先输出 <cot></cot> 包裹的简短问卷设计说明，说明你准备确认哪些方向；这是给用户看的摘要，不要输出隐藏推理、逐步思维链或冗长分析。` | 可以先输出 `<cot></cot>` 包裹的简短问卷设计说明，用来说明准备确认哪些方向；这是给用户看的摘要，不输出隐藏推理、逐步思维链或冗长分析。 |
...
触发位置：用户在 `/workspace` 写下最初线索并点击“领取登岛问卷”后，进入 `/workspace/:projectId/questionnaire` 自动生成问卷。
