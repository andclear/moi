...
- `{{character_profile}}`：更新后的岛民档案 Markdown 内容，会被替换进角色信息 YAML 生成 prompt。
...
| 登岛问卷 System | `<cot> 写 3-5 条即可：概括你抓到的核心线索、准备确认的题目方向、以及为什么需要保留“其他”选项。` | `<cot>` 写 3 到 5 条即可：概括核心线索、题目方向，以及保留“其他”选项的原因。 |
...
| 开场白 System | `不要输出 Markdown，不要输出 <cot>，不要输出 JSON 以外的内容。` | 不要输出 Markdown，不要输出 `<cot>` 设计说明标签，不要输出 JSON 以外的内容。 |
...
必须把用户最初输入、TA 的性别、TA 的年龄写进档案，优先放在 ## 最初的印象；如果年龄未填写，必须明确写“年龄：未填写”。
性别和年龄是用户直接提供的基础事实，不能省略、改写成不确定，也不能在后续内容中丢失。
...
触发位置：`/workspace/:projectId/profile` 下的“内心独白”“这不是TA”“叙事碎片”“日记破译”四个子阶段。
...
来源：`src/prompts/profilePrompts.ts` 的 `buildProfileDossierUpdateMessages(input)`。

触发位置：`/workspace/:projectId/profile` 中完成“日记破译”后点击“更新岛民档案”。
...
必须保留并延续当前岛民档案中的用户最初输入、性别、年龄。性别和年龄是用户直接提供的基础事实，不得丢弃；年龄未填写时继续写“年龄：未填写”。
每段 1-3 句即可，避免长篇解释。
...
## 2.2 角色信息 YAML 生成 Prompt

来源：`src/prompts/characterProfilePrompts.ts` 的 `buildCharacterProfileYamlMessages(characterProfile)`。

触发位置：`/workspace/:projectId/profile` 中“更新岛民档案”完成后后台静默生成；也可在岛民档案抽屉中点击“生成角色信息”或失败后重试。
